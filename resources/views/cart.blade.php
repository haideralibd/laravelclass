    ...
    <tbody>
      @foreach($data['temp_orders'] as $temp_order) 
        ...
        <tr id="tid_{{$temp_order->temp_order_row_id}}">
          ...
          <td> 
            <a href="javascript:void(0)" temp_order_row_id="{{ $temp_order->temp_order_row_id }}" class="btn btn-danger remove-item" data-toggle="modal" data-target="#confirm" />Delete
            </a>
            <input type="hidden" name="temp_order_row_id[]" value="{{ $temp_order->temp_order_row_id }}" /> 
           </td>
        </tr>
...
$(document).ready(function() {
 $("#qty_textbox").change(function(){ 
  ...
  //$('#update_cart').on('click', '.remove-item', function(e){
